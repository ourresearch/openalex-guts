...
run_queue_work: python -m scripts.queue --run --table=work --chunk=$QUEUE_WORKER_CHUNK_SIZE --name=queue_record-worker$DYNO-${i} --randstart
run_queue_concept: python -m scripts.queue --run --table=concept --method=process --chunk=10
run_queue_concept_wiki: python -m scripts.queue --run --table=concept --method=save_wiki --chunk=10

...
entities = [
    "works", "authors", "sources", "institutions", "topics", "funders", "publishers"
]
...
for e in entities:
    ...
    num_created = requests.get(
        f'https://api.openalex.org/{e}?filter=from_created_date:{yesterday}&api_key={api_key}'
    ).json().get('meta').get('count')

    num_updated = requests.get(
        f'https://api.openalex.org/{e}?filter=from_updated_date:{yesterday}&api_key={api_key}'
    ).json().get('meta').get('count')
    ...
    db.engine.execute(
        text(f'insert into log.{e}_per_day (day, num_created, num_updated) values (:day, :created, :updated)').bindparams(
            day=yesterday, created=num_created, updated=num_updated
        ).execution_options(autocommit=True)
    )

    if e == "works" and (int(num_created) == 0 or int(num_updated) == 0):
        # send warning email

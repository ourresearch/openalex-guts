...
"""
Run with: heroku local:run python -m scripts.queue_work_store_vectors --chunk=100
"""


def process_embeddings(work):
    print(f"Processing {work.id}")
    store_embeddings(work)
    db.session.execute('''
    ...
    def worker_run(self, **kwargs):
        ...
        if single_id:
            ...
            while num_updated < limit:
                ...
                logger.info(f'running process_embeddings on {works}')
                try:
                    store_embeddings(works)

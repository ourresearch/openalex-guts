...
def find_wikidata_id_for_source(display_name):
    ...
    if r.status_code == 200:
        ...
        if len(response["search"]) == 1:
            # check if the response has instance of (P31) is scientific journal Q5633421
            ...
            if r2.status_code == 200:
                response2 = r2.json()
                if is_instance_of_scientific_journal(response2) or has_issn_or_issnl(response2) or has_source_words_in_description(response2):
                    wikidata_id = f"https://www.wikidata.org/entity/{wikidata_shortcode}"
                    return wikidata_id
                else:
                    wikidata_id = f"https://www.wikidata.org/entity/{wikidata_shortcode}"
                    print(f"Not a scientific journal {wikidata_id}")
                    return None

...
def get_objects(entity_type, object_ids):
    ...
    if entity_type == "work":
        objects = db.session.query(models.Work).options(
            ...
            selectinload(models.Work.safety_journal).selectinload(models.Source.publisher_entity).selectinload(models.Publisher.self_and_ancestors).raiseload('*'),
            selectinload(models.Work.safety_journal).selectinload(models.Source.institution).raiseload('*'),
            selectinload(models.Work.safety_journal).selectinload(models.Source.publisher_entity).raiseload('*'),
            selectinload(models.Work.safety_journal).raiseload('*'),

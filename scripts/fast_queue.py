...
def get_objects(entity_type, object_ids):
    ...
    if entity_type == "work":
        objects = db.session.query(models.Work).options(
            ...
            # selectinload(models.Work.topics).selectinload(models.WorkTopic.topic).selectinload(models.Topic.subfields).raiseload('*'),
            # selectinload(models.Work.topics).selectinload(models.WorkTopic.topic).selectinload(models.Topic.fields).raiseload('*'),
            # selectinload(models.Work.topics).selectinload(models.WorkTopic.topic).selectinload(models.Topic.domains).raiseload('*'),

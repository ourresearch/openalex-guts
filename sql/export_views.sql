...
--- DISTSTYLE key
--- distkey (ConferenceSeries)
--- sortkey (ConferenceSeries)
as (
        with
...
as (
    select
        work_id as PaperId,       --- FOREIGN KEY REFERENCES papers.papers_id
        abstract as IndexedAbstract    --- Inverted index, see https://en.wikipedia.org/wiki/Inverted_index
 from mid.abstract)
with no schema binding;

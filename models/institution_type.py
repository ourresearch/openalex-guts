...
class InstitutionType(db.Model):
    ...
    institution_type_id = db.Column(db.Text, primary_key=True)
    ...
    @cached_property
    def id(self):
        return self.institution_type_id

    @property
    def openalex_id(self):
        return f"https://openalex.org/institution-types/{self.id}"

    def store(self):
        bulk_actions, new_entity_hash = create_bulk_actions(self, INSTITUTION_TYPES_INDEX)
    ...
    def __repr__(self):
        return f"<Institution type {self.id} {self.display_name}>"
